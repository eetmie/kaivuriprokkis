...
- Axis-rotation helpers (`extract_axis_rotation`, `project_to_rotation_axes`, `propagate_base_rotation`) — pure quaternion math, belongs in `math.py`.
...
- **Joint order is fixed**: `slew → lift_boom → tilt_boom → scoop` (4 DOF). The `RobotConfig` YAML loader in `config.py` can stop accepting arbitrary chain lengths and hard-code the four named links. Catch shape mismatches at load time with a clear error instead of silently propagating wrong-sized arrays through FK.
