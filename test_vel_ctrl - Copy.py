...
Usage:
    ...
    sudo python test_vel_ctrl.py --vel-max-degps 15 --vel-kp 0.06
"""
...
ASSIST_MAX_TRIM = 0.20
ACTIVE_COMMAND_FLOOR = 0.05
JOYSTICK_ZERO_EPS = 1e-3
...
class JointVelocityController:
    def __init__(
        ...
        self.active_floor_sign: dict[str, float] = {name: 0.0 for name in VEL_CTRL_JOINTS}
        self.pid: dict[str, PIDController] = {
            name: PIDController(
                kp=kp,
                ki=ki,
                kd=0.0,
    ...
    def apply(
        ...
        *,
        ...
        for name, vel_idx in VEL_CTRL_JOINTS.items():
            ...
            if abs(joystick_cmd) < JOYSTICK_ZERO_EPS:
                self.pid[name].reset()
                self.target_dps[name] = 0.0
                self.active_floor_sign[name] = 0.0
                out[name] = 0.0
                continue
            ...
            if abs(target) < self.deadband:
                ...
                feedback = self.pid[name].compute(desired, actual, dt=dt)
            ff_pos, ff_neg = self.ff.get(name, (0.0, 0.0))
            feed_forward = (ff_pos if joystick_cmd > 0.0 else ff_neg) * joystick_cmd
            if assist:
            ...
            valve_cmd = self._apply_active_command_floor(name, valve_cmd, joystick_cmd)
            out[name] = float(np.clip(valve_cmd, -1.0, 1.0))
        return out

    def _apply_active_command_floor(self, name: str, valve_cmd: float, joystick_cmd: float) -> float:
        joystick_sign = math.copysign(1.0, joystick_cmd)
        self.active_floor_sign[name] = joystick_sign

        # Keep active velocity commands on the user's requested side of the valve.
        # If PI tries to back through zero, hold at the floor instead of re-entering
        # the long deadband/stiction region or briefly commanding the opposite side.
        along_direction = valve_cmd * joystick_sign
        if along_direction < ACTIVE_COMMAND_FLOOR:
            along_direction = ACTIVE_COMMAND_FLOOR
        return joystick_sign * along_direction
    ...
    def reset(self) -> None:
        for pid in self.pid.values():
            pid.reset()
        for name in self.target_dps:
            self.target_dps[name] = 0.0
        for name in self.active_floor_sign:
            self.active_floor_sign[name] = 0.0


def parse_args():
    parser = argparse.ArgumentParser(description="Raw direct-drive / velocity-PI prototype test")
    parser.add_argument("--vel-kp",        type=float, default=0.04,  help="Proportional gain (cmd / deg_s_err)")
    parser.add_argument("--vel-ki",        type=float, default=0.008, help="Integral gain")
    parser.add_argument("--vel-ki-max",    type=float, default=0.4,   help="I-term anti-windup clamp")
    parser.add_argument("--vel-deadband",  type=float, default=2.0,   help="Error deadband (deg/s)")
    parser.add_argument("--vel-max-degps", type=float, default=20.0,  help="Full joystick = this many deg/s")
